...
\begin{document}
	...
	\begin{figure}
		...
		\label{fig:overview}
	\end{figure}
	%TODO Rewrite, clearly indicate the system vs the controller, and this is about designing the controller.
...
%TODO Rewrite explanation. Controller -> System interactions
...
\end{itemize}
	...
	%TODO Rewrite : System -> Controller interactions
...
\item WaferTreated($W$) - Wafer $W$ is treated by the projector.
	...
	%TODO Add explanation of what is in the chapter.
	%TODO Fix line break in the middle of functions.
	\section{Liveness requirement}
	\begin{itemize}
		\reqb{SR1}{After a RobotFromInput(w) happened a TreatWafer(w) should eventually happen, eventually happened by RobotToOutput(TreatedWafer(w))}
	\end{itemize}
	\section{Sluice requirements}
	\begin{itemize}
...
\end{itemize}		
    ...
    \textbf{MCF1}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}. [\mathit{Wafer}FromInput(w)]\\
    ...
    \textbf{MCF3}
    \begin{multline*}
        [true^{\star}] \forall s \in SluiceID. [PumpVacuum(s) \cdot \overline{ReleaseVacuum}^{\star} \cdot OpenOuterDoor(s)]true
    \end{multline*}
    ...
    \textbf{MCF6a1}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}, s \in SluiceID.[RobotFromSluice(Ra,s,w) \cdot \overline{RobotReset(Ra)}^{\star}\\
         \cdot CloseOuterDoor(s)]false
    \end{multline*}

    \textbf{MCF6a2}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}, s \in SluiceID.[RobotToSluice(Ra,s,w) \cdot \overline{RobotReset(Ra)}^{\star} \\
         \cdot CloseOuterDoor(s)]false
    \end{multline*}
	
    \textbf{MCF6b1}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}, s \in SluiceID.[RobotFromSluice(Rb,s,w) \cdot \overline{RobotReset(Rb)}^{\star}\\
         \cdot CloseInnerDoor(s)]false
    \end{multline*}

    \textbf{MCF6b2}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}, s \in SluiceID.[RobotToSluice(Rb,s,w) \cdot \overline{RobotReset(Rb)}^{\star} \\
         \cdot CloseInnerDoor(s)]false
    \end{multline*}

    \textbf{MCF71}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}, s \in SluiceID.[OuterDoorClosed(s) \cdot \overline{OuterDoorOpened}^{\star} \\
    ...
    \textbf{MCF81}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}, s \in SluiceID.[InnerDoorClosed(s) \cdot \overline{InnerDoorOpened(s)}^{\star} \\
    ...
    \textbf{MCF9a}
    \begin{multline*}
        [true^{\star}] \forall w1,w2 \in \mathit{Wafer}, s in SluiceID.[RobotToSluice(Ra,s,w1) \\
         \cdot \overline{RobotFromSluice(Rb,s,w1)}^{\star} \cdot  \exists w2 \in \mathit{Wafer}. RobotToSluice(Ra,s,w2)]false
    \end{multline*}

    \textbf{MCF9b}
    \begin{multline*}
        [true^{\star}] \forall w1 \in \mathit{Wafer}, s \in SluiceID.[RobotToSluice(Rb,s,w1)\\
    ...
    \textbf{MCF11}
    \begin{multline*}
        [true^{\star}] \forall p \in \mathbb{N}, r \in RobotID, w \in \mathit{Wafer}. [RobotToRack(r,p,w)\\
         \cdot \overline{RobotFromRack(r,p,w)}^{\star} \cdot RobotToRack(r,p,w)]false
    \end{multline*}

    \textbf{MCF12}
    \begin{multline*}
        [true^{\star}] \forall s \in SluiceID [SluiceBroken(s) \cdot true^{\star} \cdot (CloseInnerDoor(s) \\
    ...
    \textbf{MCF13a}
    \begin{multline*}
        [true^{\star}] \forall p \in \mathbb{N}, w \in \mathit{Wafer}.[\overline{RobotToRack(Rb,p,w}^{\star} \cdot RobotFromRack(Rc, p, w)]false
    \end{multline*}

    \textbf{MCF13b}
    \begin{multline*}
        [true^{\star}] \forall p \in \mathbb{N}, w \in \mathit{Wafer}.[\overline{RobotToRack(Rc,p,w}^{\star} \cdot RobotFromRack(Rb, p, w)]false
    \end{multline*}

    \textbf{MCF14}
    \begin{multline*}
        [true^{\star}] \forall p \in \mathbb{N}, w \in \mathit{Wafer}. [(RobotToRack(Rc,p,w) \vee RobotFromRack(Rc,p,w))\\
         \cdot \overline{RobotReset(Rc}^{\star} \cdot (RobotRack(Rb,p,w) \vee RobotFromRack(Rb,p,w)]false
    \end{multline*}

    \textbf{MCF15}
    \begin{multline*}
        [true^{\star}] \forall p \in \mathbb{N}, w \in \mathit{Wafer}. [(RobotToRack(Rb,p,w) \vee RobotFromRack(Rb,p,w))\\
        .\overline{RobotReset(Rb}^{\star} \cdot (RobotRack(Rc,p,w) \vee RobotFromRack(Rc,p,w)]false
    \end{multline*}

    \textbf{MCF16}
    \begin{multline*}
        [true^{\star}] \forall w,w2 \in \mathit{Wafer}. [RobotToProjector(w) \cdot \overline{RobotFromProjector(w)}^{\star} \\
         \cdot RobotToProjector(w2)]false
    \end{multline*}

    \textbf{MCF17}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}. [RobotToProjector(w) \cdot \overline{Treat\mathit{Wafer}(W)}^{\star}\\
         \cdot RobotFromProjector(w)]false \\
    \end{multline*}

    \textbf{MCF18a}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{Wafer}, p \in \mathbb{N}. [RobotToRack(Rc, p, w) \cdot \overline{RobotFromRack(Rb,p,w)}^{\star} \\
         \cdot RobotToRack(Rc,p,w)]false
    \end{multline*}
    ...
    \textbf{MCF19}
    \begin{multline*}
        [true^{\star}] \forall w \in \mathit{\mathit{Wafer}}. [RobotFromProjector(w) \cdot  \forall w1 \in \mathit{Wafer}. \overline{RobotToProjector(w1)}^{\star} \\
    ...
    \textbf{Verification environment}
    The environment on which the model was checked using the modal formulas is given in table.
